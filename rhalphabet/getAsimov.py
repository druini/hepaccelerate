...
if __name__ == '__main__':
    ...
    folder = os.path.join(folder_base, args.year, args.version, args.selection, 'data_msd100to160_msdbin5_'+args.pdf+'polyDegs'+str(args.polyDegPt)+'_ttHTobb'+('_statOnly' if args.statOnly else ''))

    if not os.path.isfile( os.path.join(folder,'ttHbb_r-20to20.txt') ):
        print('running my_rhalphalib.py')
        cmd = 'python my_rhalphalib.py --msd_start 100 --msd_stop 160 -s '+args.selection+' --pdf '+args.pdf+' --polyDegPt '+str(args.polyDegPt)+' -y '+str(args.year)+' -d -v '+args.version+' -r 5 --simpleFit -j /eos/home-a/algomez/tmpFiles/hepacc/results/ --signal ttHTobb '+('--statOnly' if args.statOnly else '')
        exec_me(cmd)
    ...
    exec_me(cmd, folder=folder)

    cmd = "combine -M FitDiagnostics -m 125 --cminDefaultMinimizerTolerance 1e-2 --cminDefaultMinimizerStrategy 0 --setParameterRanges TTH_PTH_GT300=-10,10 --setParameters TTH_PTH_GT300=1  -t -1 -n _asimov_sig1_r_ttH_0_5D --redefineSignalPOIs TTH_PTH_GT300 5D_Boosted_highpt.root"
    exec_me(cmd, folder=folder)
